...
基于 [Mediabunny](https://mediabunny.dev/) 的媒体编解码工具包，用于 SwiftAV 的媒体解析与导出场景。
...
- 不负责任何画布渲染逻辑（由 `@swiftav/canvas` 负责）
- 不关心时间线 / 轨道（由 `@swiftav/timeline` 负责）
...
```ts
import { createCanvasVideoOutput } from '@swiftav/media';

// 导出专用 canvas，结合 @swiftav/canvas 渲染每一帧
...
// 在时间线循环中：
// 1. 使用 @swiftav/canvas 根据当前时间渲染一帧到 canvas
